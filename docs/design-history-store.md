...
Client (`static/chat.js`): on 409 `version_conflict`, apply `current_version` from the body and retry the mutation. Do not ask the user to reload (Capacitor / embedded clients may not support a page refresh). Every mutating call follows this sync-and-retry-once contract — `/delete_message`, `/reset_chat`, `/update_system_prompt`, `/update_memory`, `/rename_set`, `/delete_set`, and the stream endpoints `/chat` + `/regenerate` (a 409 there adopts the authoritative version and replays the same turn via `reuseLastUser`, so a prompt saved at an awkward moment heals instead of erroring). After a successful `/delete_message`, reindex remaining `data-pair-index` values from live DOM order so the next delete does not send a stale index (that 409 is `content mismatch`, not `version_conflict`). Never rewind `APP_DATA.setVersion` for the same `set_id` (in-flight `get_sets` after a mutation); the selector `change` handler syncs through `applySetVersion` for the same reason — a set switch adopts the new set's version, a same-set refresh is advance-only. Authoritative read responses (`/load_set` pages, `/history_pair`) sync the version back advance-only via `noteSetVersionFromRead`, so the client converges on the server version without user action; a refresh is only needed to pick up new JS.
