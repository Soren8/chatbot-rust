...
- **Cached-account picker.** Accounts remembered on this device appear in the login page's account dropdown. Empty password calls `POST /login/keyauth` (stored encryption key in `X-Enc-Key`) and falls back to `POST /login/remember` when that cookie belongs to the selected username. A typed password always posts `/login` (the password field and Remember checkbox stay visible). A ✕ control forgets an account locally and `POST /login/forget` revokes this device's remember token only when that cookie belongs to the forgotten username.
