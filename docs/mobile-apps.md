...
4. **Voice-mode TTS** — desktop uses `playTTS` HTML `Audio` (`playOneTtsUtterance`) with in-memory `Blob` object URLs and prefetching (`preloadDesktopTtsSentence`), preventing mid-sentence drops and inter-sentence latency over spotty remote links. Android uses the ordered `NativeVoiceTtsPlugin` `AudioTrack` queue so playback survives screen lock and WebView suspension. Native TTS decouples clip fetching (`downloaderThread`) from playback (`workerThread`) via an in-memory `audioQueue` of fully decoded `AudioClip`s. Both paths use the pre-signed `/tts_stream` URL; the server retains a generated clip for a short, bounded retry window so a truncated mobile download does not skip a sentence. Spotty-link rules (do not regress these):
   ...
   - When manual TTS replay is triggered while voice mode is active (on Android or desktop), VAD speech counters and pre-roll buffers are reset immediately (`onTtsPlaybackStarted`), `bargeInFrames` is reset to 0, and in-flight voice STT is aborted. This prevents screen taps, ambient room noise, or speech from before the button click from causing immediate false barge-in. Completed messages evaluate `isStillGenerating` as false even if another request is active, allowing reliable manual replay of any message in chat. Do not change `AudioManager` mode from TTS; `NativeMic.enterVoiceRoute` owns that for the session.
    - Audio focus co-existence and speaker routing: `NativeVoiceTtsPlugin` requests `AUDIOFOCUS_GAIN` with `USAGE_MEDIA`. While its session is active (`NativeVoiceTtsPlugin.isSessionActive()`), `NativeMicPlugin` ignores transient focus loss and avoids stealing focus back via `reclaimAudioFocus()`, which would otherwise cause Android's audio mixer to duck or silence `USAGE_MEDIA`. On playback stop or completion, `NativeVoiceTtsPlugin` abandons its focus request and calls `NativeMicPlugin.reclaimAudioFocusIfPresent()` to return focus to the microphone. In `AudioManager.MODE_IN_COMMUNICATION`, Android routes media streams to the built-in earpiece by default unless overridden; `NativeVoiceTtsPlugin` assigns `setPreferredDevice` to the built-in speaker when Bluetooth audio is not present. In `static/chat.js`, `CURRENT_AUDIO.stop()` and `stopAllTtsPlayback()` explicitly invoke `resetPlayButtonUi()` and `clearMessageTtsPlayingUi()` so manual replay button state and message highlights never become desynchronized.
