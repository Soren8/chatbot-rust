...
4. **Voice-mode TTS** — desktop uses `playTTS` HTML `Audio` (`playOneTtsUtterance`) with in-memory `Blob` object URLs and prefetching (`preloadDesktopTtsSentence`), preventing mid-sentence drops and inter-sentence latency over spotty remote links. Android uses the ordered `NativeVoiceTtsPlugin` `AudioTrack` queue so playback survives screen lock and WebView suspension. Native TTS decouples clip fetching (`downloaderThread`) from playback (`workerThread`) via an in-memory `audioQueue` of fully decoded `AudioClip`s. Both paths use the pre-signed `/tts_stream` URL; the server retains a generated clip for a short, bounded retry window so a truncated mobile download does not skip a sentence. Spotty-link rules (do not regress these):
   ...
   - `stopPlaybackInternal(true)` emits `playbackState: 'ended'` only when a session was actually active. A teardown `stop()` before `beginSession()` must stay silent — its async stale `'ended'` can land on the new session's listener after a cold app start (kill + relaunch) and kill the session at birth, leaving TTS silent until the next message. In addition, `NativeVoiceTtsPlugin` tracks a monotonic generation ID returned by `beginSession` and included in all `playbackState` events (`started`, `ended`, `error`). The JS listener filters out events from older generations and ignores `ended` events before playback has started or end-of-queue was requested, guaranteeing that stopping a prior session never kills a new or replayed playback at birth.
    - Unified audio output and focus co-existence: Android unifies all TTS playback (manual play button, sentence clicks, and voice mode streaming) through the native `NativeVoiceTtsPlugin` `AudioTrack` pipeline, eliminating dual-engine conflicts with WebView HTML5 audio. Handheld voice mode is full-duplex communication: capture uses `VOICE_COMMUNICATION` (hardware AEC/AGC) under `MODE_IN_COMMUNICATION` + speakerphone, and TTS output uses `USAGE_VOICE_COMMUNICATION` / `CONTENT_TYPE_SPEECH` with matching focus attributes (same-attributes player/focus per the audio-focus guide); standalone TTS outside voice mode keeps `USAGE_MEDIA` with the `STREAM_MUSIC` legacy mapping. The communication track omits `setLegacyStreamType` (it overwrites attrs per AOSP audio attributes). The track recreates on voice enter/exit even at the same rate and reuses otherwise, so steady playback avoids per-sentence focus/binder churn; the holder releases only the focus it requested. The preferred-device speaker hack applies only where the owned route holds communication (`TtsAudioPolicy` from actual `isRouteActive`, never blind foreground/mode); a Bluetooth-skipped route stays `MODE_NORMAL` with media playback. While TTS is active, `NativeMicPlugin` avoids stealing focus back via `reclaimAudioFocus()`. On playback completion (`isEndOfQueue` drained) or explicit user stop (`stopPlaybackInternal(true)`), `NativeVoiceTtsPlugin` releases audio focus and notifies `NativeMicPlugin` to reclaim focus. In `static/chat.js`, all TTS paths on Android native route to `playNativeVoiceModeTts`, and `CURRENT_AUDIO.stop()` and `stopAllTtsPlayback()` explicitly invoke `resetPlayButtonUi()` and `clearMessageTtsPlayingUi()`. The app never writes system stream volumes: entering/exiting voice mode holds `MODE_IN_COMMUNICATION` + speakerphone only (no max/restore, so a user-lowered level survives the session) and TTS acquires matching focus without touching any stream; hardware keys follow the active stream (call stream in voice mode, music standalone; call steps/minimum are device-dependent) with no activity pin and no remote `VolumeProvider`/`MediaSession` workaround. Pause releases the route so a real call owns audio. Off-device harness coverage proves the owned route/policy composition selects communication vs media, recreates on usage change, and writes no volumes; it does not prove actual Android routing, which needs device validation.
