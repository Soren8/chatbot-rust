...
2. `NativeMicUtteranceVAD` in `chat.js` (shipped path — not Silero):
   ...
   - One `stopAllTtsPlayback` path for play/stop, message click, Send/Stop, barge-in, and disabling voice mode (desktop HTML audio + native `AudioTrack`)
    - Same `/tts` then `/tts_stream/{token}` as desktop. `NativeVoiceTtsPlugin` plays that GET into `AudioTrack` + `USAGE_VOICE_COMMUNICATION` (WebView HTML audio cannot give AEC a speakerphone reference). Re-request audio focus and unmute `STREAM_VOICE_CALL` before play — the Unrestricted-battery dialog (and other activities) steal focus and otherwise leave voice-mode TTS silent while regular HTML TTS still works. Stream PCM with a ~400 ms preroll; retry a failed GET; do not abort the queue on one dropped clip.
