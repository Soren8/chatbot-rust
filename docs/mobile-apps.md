...
4. **Voice-mode TTS** — desktop uses `playTTS` HTML `Audio` (`playOneTtsUtterance`) with in-memory `Blob` object URLs and prefetching (`preloadDesktopTtsSentence`), preventing mid-sentence drops and inter-sentence latency over spotty remote links. Android uses the ordered `NativeVoiceTtsPlugin` `AudioTrack` queue so playback survives screen lock and WebView suspension. Native TTS decouples clip fetching (`downloaderThread`) from playback (`workerThread`) via an in-memory `audioQueue` of fully decoded `AudioClip`s. Both paths use the pre-signed `/tts_stream` URL; the server retains a generated clip for a short, bounded retry window so a truncated mobile download does not skip a sentence. Spotty-link rules (do not regress these):
   - The JS sentence pump requeues a sentence on **any** transient failure — network error, stall timeout, 429/5xx (`MAX_TTS_SENTENCE_RETRIES`, backoff before the delayed pump). Exhausted retries end the session with `appendMessage('Voice output failed. Try again.', 'error-message')` plus `console.error`/`reportVoice` diagnostics, and never advance to later sentences. During active voice mode, desktop persists retries instead of ending the session; 401 redirects to `/login`.
   - `fetchVoiceRetry` distinguishes its own 60s stall timeout from a user stop (`userAborted`): stalls retry, user stops do not.
    - The native downloader decodes complete clips into `AudioClip` before pushing to `audioQueue`: Ogg-Opus responses (the server default, `tts_codec: opus`) decode through the streaming pure-Java `audio/OggOpusStreamDecoder` (concentus, no NDK), WAV fallback responses through the existing WAV decoder. The decoder decodes at 48 kHz and downsamples to the OpusHead rate because Concentus 1.0.2 drops the SILK layer of hybrid packets at any resampling output rate (see [audio-decoder-check.md](audio-decoder-check.md)); the app still sees the clip's own rate. Off-device coverage of exactly that decode step (pre-`AudioTrack`, not run by `cargo test`) lives in the trusted executor operation `android-opus-decoder-check`: it compares the decoder against a libopus reference decode of real server-encoder output across network chunk shapes. Fixtures, tolerances, and the rollout/run commands are in [audio-decoder-check.md](audio-decoder-check.md). An interrupted download fails cleanly and retries with backoff (`CLIP_RETRY_BACKOFF_MS`) while the session generation is active; partial audio is never written to `AudioTrack`, preventing mid-sentence cutoffs. Desktop similarly buffers full clips into in-memory `Blob`s and creates `blob:` URLs before passing to `<audio>` (browsers decode Ogg-Opus natively), revoking object URLs after playback or on stop.
   - `stopPlaybackInternal(true)` emits `playbackState: 'ended'` only when a session was actually active. A teardown `stop()` before `beginSession()` must stay silent — its async stale `'ended'` can land on the new session's listener after a cold app start (kill + relaunch) and kill the session at birth, leaving TTS silent until the next message. In addition, `NativeVoiceTtsPlugin` tracks a monotonic generation ID returned by `beginSession` and included in all `playbackState` events (`started`, `ended`, `error`). The JS listener filters out events from older generations and ignores `ended` events before playback has started or end-of-queue was requested, guaranteeing that stopping a prior session never kills a new or replayed playback at birth.
    - Unified audio output and focus co-existence: Android unifies all TTS playback (manual play button, sentence clicks, and voice mode streaming) through the native `NativeVoiceTtsPlugin` `AudioTrack` pipeline, eliminating dual-engine conflicts with WebView HTML5 audio. `NativeVoiceTtsPlugin` requests `AUDIOFOCUS_GAIN` with `USAGE_MEDIA`. While active, `NativeMicPlugin` avoids stealing focus back via `reclaimAudioFocus()`. On playback completion (`isEndOfQueue` drained) or explicit user stop (`stopPlaybackInternal(true)`), `NativeVoiceTtsPlugin` releases audio focus and notifies `NativeMicPlugin` to reclaim focus. In `AudioManager.MODE_IN_COMMUNICATION`, Android routes media streams to the built-in speaker when no headset or Bluetooth audio is connected (`findBuiltInSpeaker()` queries `AudioManager.GET_DEVICES_OUTPUTS` as required for `AudioTrack.setPreferredDevice`). In `static/chat.js`, all TTS paths on Android native route to `playNativeVoiceModeTts`, and `CURRENT_AUDIO.stop()` and `stopAllTtsPlayback()` explicitly invoke `resetPlayButtonUi()` and `clearMessageTtsPlayingUi()`. The app never writes system stream volumes automatically: entering/exiting voice mode holds `MODE_IN_COMMUNICATION` + speakerphone only (no max/restore, so a user-lowered level survives the session) and TTS acquires media focus without touching `STREAM_MUSIC`; only explicit user volume gestures write levels. `MainActivity` requests `STREAM_MUSIC` for foreground keys (`setVolumeControlStream`), but that hint alone is unreliable (observed misses both foreground and background), so the owned voice-mode session (`VoiceModeSessionCoordinator` + `VoiceModeVolumeSession`) additionally holds a remote-volume `MediaSession` (absolute provider on the music scale, playing state for the whole session including between clips, no callback/metadata/actions, never attached to the CallStyle stop notice) so background/locked keys also drive `STREAM_MUSIC`, requesting the standard panel with re-synced display. A local session does not fix the call-stream precedence, so the remote provider is a platform workaround for local audio, not canonical local-media behavior. Pause releases it so a real call owns the keys; stale provider callbacks after replacement are ignored by generation. Off-device harness coverage proves the owned composition forwards keys/slider to music without automatic writes; it does not prove actual Android routing, which needs device validation.
   - First-sentence streaming latency: In voice mode (on Android and desktop), TTS begins streaming and playing audio as soon as the initial complete sentence arrives over text, without waiting for the full response to finish generating. `splitSentences` identifies paragraph breaks (`\n\n`), colons preceding line breaks (`:\n`), and list items (`\n-`, `\n*`, `\n1.`) as sentence boundaries, and `sanitizeForTTS` preserves newlines so paragraph boundaries are not collapsed into spaces. `discoverSentences` (native) and `discoverAbsolute` (desktop) distinguish trailing in-flight fragments from completed earlier sentences: earlier sentences are pushed to the TTS pump immediately while generating, and only the trailing fragment waits for `sentenceEndsWithTerminator`, which holds extendable endings (trailing colon, digit period, short all-caps initialism, known abbreviations/honorifics) until their continuation arrives; a chunk that only appends punctuation/closers to an already-queued sentence is ignored instead of repeated. `NativeVoiceTtsPlugin` sets the `AudioTrack` buffer size to 64KB (down from 256KB) to avoid hardware preroll thresholds delaying the first sample.
...
- **Voice telemetry** (`reportVoice` in `static/chat.js`, `ClientLogReporter.report` in `NativeMicPlugin` / `NativeVoiceTtsPlugin`) — voice-session start/stop, `enterVoiceRoute` result, first-PCM-frame, utterance end (chunks/speechMs), STT upload start (format/bytes) / ok (text length only) / fail, mic start failures, TTS clip give-up and non-retryable codes. Hot paths (dropped PCM frames, STT-overlap skips) report at most once per 10–30s. Metadata only: never transcripts, audio bytes, cookies, tokens, or URLs. Debug builds only; release stays silent.
- Useful checks: `adb logcat -s ClientLogReporter` shows upload attempts and result codes; server side shows the sanitized payload. Voice sessions emit a few lines per turn (boundaries + STT summary); crashes and failures always report.
- **No-adb collection:** every `Logger.report` uploads asynchronously to `POST /client_logs`, which lands in the webserver log at warn level (`source=android`, `client_log=...`). Grep host logs with `grep "client log" | grep -i voice`. The reporter needs a live session cookie (guest `session=` is enough) and the server image must include the `/client_logs` route — pull/rebuild `webserver` if voice reports never appear.
