...
- **Sessions & storage** – Session state lives in a Rust-managed in-memory store (guest RAM mirror only). Opt-in **remember tokens** ("Remember this computer for 30 days", unchecked by default) survive restarts: `GET /` silently restores a remembered session for guest visitors (rotating the token), `POST /login/remember` is the CSRF-guarded manual restore, `POST /login/keyauth` is the password-free cached-key login behind the login page's account dropdown (keyed by plaintext username, written only when "Remember this computer" is checked; a ✕ forget control deletes a slot locally and revokes the token), and tokens are durable rotating device secrets (server stores only `sha256(token)` under `data/remember_tokens/`; previous-generation grace for concurrent tabs, older replays revoke the family). The token restores the session only and never bypasses the per-request `X-Enc-Key` gate. Chat history for authenticated users is stored in an embedded **redb** database of AEAD ciphertext via `HistoryService`: a sealed header + manifest, per-pair text chunks, and extracted image/thumbnail blobs (see [design-history-store.md](design-history-store.md) and [design-history-chunks.md](design-history-chunks.md)). Pre-redb `user_sets/{user}/sets.json` is **migration-only** and lives permanently in `chatbot-core::legacy_sets_json` (lazy import into redb, then `sets.json.migrated.bak`). Whole-set `SETS_BLOB` rows migrate lazily to chunks on the first payload op. Anonymous flows remain ephemeral/RAM-only.
- **Tooling & operations** – Development and CI rely on Docker. The compose `tests` service runs `cargo test` for the workspace (including `js_syntax`, which parses first-party `static/*.js` with `oxc_parser`); `docker compose run --rm tests`. The runtime image builds the Axum binary. Build caches, Cargo registries, and generated logs are mounted under `temp/` per `AGENTS.md`.
...
- **Authentication & Security**
  ...
  - [x] Remember-this-computer device tokens (opt-in, 30 days, CSRF-guarded restore endpoint, rotating single-use secrets with replay-driven family revocation; hashed-only server side, HttpOnly cookie). The client key cache is slotted per account by unsalted SHA-256 of the username, enabling silent resume plus a local saved-account picker without storing account names in the browser.
