...
- **Authentication & Security**
  ...
  - [x] Remember-this-computer device tokens (30 days, checked by default) with silent session restore on app entry, a cached-account login dropdown (username + forget; password still required), and a true opt-out via the unchecked box (revokes the device's token).
