...
- **Authentication & Security**
  ...
  - [x] Remember-this-computer device tokens (30 days, checked by default) with silent session restore on app entry, a cached-account login dropdown (password-free via per-account remember cookies until ✕; forget control), and a true opt-out via the unchecked box or ✕ (revokes the device's token). The Fernet key is not a login credential.
