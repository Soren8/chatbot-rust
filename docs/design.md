...
- **Providers & chat flow** – The chat pipeline is orchestrated in Rust and configured through `.config.yml`. Provider implementations (OpenAI-compatible, XAI) live in `chatbot-server` and use async traits with streaming support, exercised by cargo integration tests. XAI providers support optional `xai_zdr: true`, which sends `store: false` on the Responses API (no server-side conversation retention) and logs a warning if the response `x-zero-data-retention` header is not `true`. Full team-level Zero Data Retention is enabled in the [xAI Console](https://console.x.ai/) and is independent of this flag. OpenAI-compatible providers retry upstream `429 Too Many Requests` before the stream starts: per-provider `rate_limit_retries` (default 1, 0 disables) extra attempts whose wait honors the upstream `Retry-After` hint capped by `rate_limit_max_wait_secs` (default 5s), falling back to short exponential backoff; retries never happen mid-stream, and exhausted retries surface the provider error body exactly as before.
