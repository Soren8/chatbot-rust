...
Android build flavor is authoritative for the default server origin. Root `capacitor.config.json` defines both emulator and physical endpoints under `serverUrls`; Gradle projects the selected endpoint into `R.string.server_url`. The WebView, credential cookies, client logging and Android Auto use that resource through `ServerUrlResolver`, with a validated persisted user override (`ServerUrlSetting`) applying on top after install; switching purges the old origin's session + credential cookies (persisting only after the purge is acknowledged) and re-scopes keystore credential slots per origin so nothing transfers.
...
- **Privacy Modes**
  - [x] Saved sets currently support **Private** (default, including legacy sets) and **Non-private**. The next policy model adds **Standard** between them: `private` = local or verified no-retention processing; `standard` = reputable third parties with limited/anonymized retention; `non_private` = everything else. Both existing modes retain per-user-key encrypted history; encryption does not authorize outbound transmission. The three-level design keeps chats defaulting to `private` and unclassified model, model-native search, Brave search, STT and TTS destinations defaulting to `non_private`. Eligibility is a lattice: a chat may use a destination only at its level or a stricter one (`private` → private only; `standard` → private or standard; `non_private` → any), subject to tier/access checks. ZDR LLM endpoints, xAI search on ZDR teams (with an undocumented-upstream caveat), and Brave standard 90-day / enterprise-ZDR search belong to Standard only when the full route qualifies; a provider flag alone is not proof.
  ...
  - [x] Shared chat UI currently shows the two implemented modes, waits for saved-set policy before enabling send/search/voice, gates incompatible destinations, and requires deliberate confirmation to switch to Non-private. See [design-privacy.md](design-privacy.md) for voice legacy/guest behavior and limits.
