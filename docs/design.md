...
- **Authentication & Security**
  ...
  - [x] Remember-this-computer device tokens (30 days, checked by default) with silent session restore on app entry, a cached-account login dropdown (password-free via the remember cookie when it matches that account; forget control), and a true opt-out via the unchecked box (revokes the device's token).
