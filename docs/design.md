...
- **Speech & ML Infrastructure** – To optimize for VRAM efficiency and minimize disk footprint, all Python-based ML workloads (STT, TTS, and potentially local LLM inference) are consolidated into a single `chatbot-cuda` microservice. This monolithic approach allows models to share the same CUDA context and PyTorch runtime, significantly reducing static memory overhead compared to running separate containers for each model. It uses `uv` for fast dependency management and shares CUDA library caches via Docker volumes.
