...
- **Brave Search MCP integration** – Non-XAI providers (OpenAI-compatible API) support web search via a Brave Search MCP sidecar. When `web_search=true` is requested, the server performs a two-phase call: first a non-streaming call with OpenAI function-calling tools to let the model decide whether to search, then (if a `brave_web_search` tool call is returned) the query is forwarded to the MCP server via JSON-RPC, and the search results are injected as a tool message before the final streaming response. XAI models retain their native search capability unchanged. The MCP sidecar URL is configured via the `BRAVE_MCP_URL` environment variable. Additional Brave tools available for future extension include `brave_local_search`, `brave_video_search`, `brave_image_search`, `brave_news_search`, and `brave_summarizer`.
