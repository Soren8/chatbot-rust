...
pub struct LegacySetsStore {
    sets_dir: PathBuf,
    default_system_prompt: String,
}

impl LegacySetsStore {
    ...
    pub fn sets_json_path(&self, username: &str) -> Result<PathBuf, PersistenceError> {
        let username = LegacySetsStore::normalise_username(username)?;
    ...
    fn read_sets(
        ...
        for (name, mut meta) in raw.into_iter() {
            if let Ok(valid) = LegacySetsStore::normalise_set_name(Some(&name)) {
    ...
    fn encrypt(
        ...
        LegacySetsStore::encrypt_bytes(content.as_bytes(), encryption)
    }
    ...
    fn decrypt(
        ...
        let bytes = LegacySetsStore::decrypt_bytes(content, encryption)?;
    ...
    pub fn create_set(
        ...
        let username = LegacySetsStore::normalise_username(username)?;
        let set_name = LegacySetsStore::normalise_custom_set_name(set_name)?;
    ...
    pub fn delete_set(
        ...
        let username = LegacySetsStore::normalise_username(username)?;
        let set_name = LegacySetsStore::normalise_set_name(Some(set_name))?;
    ...
    pub fn rename_set(
        ...
        let username = LegacySetsStore::normalise_username(username)?;
        let old_name = LegacySetsStore::normalise_set_name(Some(old_name))?;
        let new_name = LegacySetsStore::normalise_custom_set_name(new_name)?;
